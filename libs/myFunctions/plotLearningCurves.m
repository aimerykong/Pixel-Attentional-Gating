...
if ~isempty(stats) && isfield(stats, 'val') && ~isempty(stats.val)
    ...
    for p = plots
        ...
        elseif numel(plots) <= 15 
            subplot(3, 5, find(strcmp(p,plots))) ;
        ...
        if isempty(strfind(lower(p), 'acc'))
            ...
            title(sprintf('%s ts%.4f (%d) tr%.4f (%d) ', p(1:min(10,length(p))), min(values(2,:)), minIdx, min(values(1,:)),minIdxTr), 'Interpreter', 'none') ;
            ...
            title(sprintf('%s ts%.4f (%d) tr%.4f (%d) ', p(1:min(10,length(p))), max(values(2,:)), maxIdx, max(values(1,:)), maxIdxTr), 'Interpreter', 'none') ;
            legend(leg{:},'location', 'SouthOutside') ;
            grid on ;
        end
    end
