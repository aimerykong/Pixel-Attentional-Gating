...
switch lower(opts.loss)
    ...
    case{lower('normalRegLoss')}
        assert(labelSize(3) == inputSize(3)) ;
        instanceWeights = repmat(single(sum(c.^2,3)>0.9),[1 1 3]);
    case{lower('normalDotProdLoss'), lower('normalACosLoss'), lower('normalACosLoss_inv')}
        assert(labelSize(3) == inputSize(3)) ;
        instanceWeights = single(sum(c.^2,3)>0.5);
    otherwise
...
if nargin <= 2 || isempty(dzdy)
    switch lower(opts.loss)
        ...
        case lower('depthRegLoss')
            ...
            t = t.^2;
            t = 0.5/nn*sum(t(:));
            %figure(1001);subplot(1,2,1);imagesc(c);axis off image;colorbar;subplot(1,2,2);imagesc(x);axis off image;colorbar;
